...
class DatabaseValidator {
  ...
  async validateConnectivity() {
    ...
    try {
      const { data, error } = await supabase.from('user_session').select('count').limit(1);
  ...
  async validateRLS() {
    ...
    try {
      ...
      const { data, error } = await supabase.from('user_session').select('id').limit(5);
  ...
  async validateFunctions() {
    ...
    try {
      ...
      const { data: validateResult, error: validateError } = await supabase.rpc(
        'validate_session',
        { token_to_check: 'test_validation_token' }
      );
  ...
  async validateDataHealth() {
    ...
    try {
      ...
      const expiredActive =
        stats?.filter((s) => s.is_active && new Date(s.expires_at) < new Date())?.length || 0;
      ...
      if (expiredActive > 0) {
        console.warn(
          `   ⚠️ Found ${expiredActive} expired but still active sessions, consider running cleanup`
        );
      }
  ...
  async quickHealthCheck() {
    ...
    try {
      const { data: sessionStats, error } = await supabase.rpc('cleanup_expired_sessions');
...
if (require.main === module) {
  ...
  switch (command) {
    case 'quick':
      ...
      validator.quickHealthCheck().then((result) => process.exit(result ? 0 : 1));
      break;

    case 'full':
    default:
      console.log('Executing full validation...');
      validator.runFullValidation().then((result) => process.exit(result ? 0 : 1));
      break;
  }
}
