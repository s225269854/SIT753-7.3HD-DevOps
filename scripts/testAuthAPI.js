  ...
  async testErrorHandling() {
    ...
    const errorTests = [
      ...
      },
      {
        ...
        test: () =>
          axios.post(`${this.baseURL}/api/auth/refresh`, {
      ...
      },
      {
        name: '无效access token',
        test: () =>
          axios.get(`${this.baseURL}/api/auth/profile`, {
    ...
    for (const errorTest of errorTests) {
      try {
        ...
        if (error.response && error.response.status >= 400) {
          this.log.success(
            `${errorTest.name}: Correctly returned error (${error.response.status})`
          );
          passedTests++;
        } else {
...
if (require.main === module) {
  ...
  switch (command) {
    case 'quick':
      tester.quickTest().then((result) => process.exit(result ? 0 : 1));
      break;

    case 'full':
    default:
      tester.runFullTest().then((result) => process.exit(result ? 0 : 1));
      break;
  }
}
