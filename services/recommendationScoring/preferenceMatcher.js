...
const DIETARY_RULES = [
  {
    ...
    conflictMessage:
      'Recipe appears to include meat, which conflicts with your vegetarian preference.',
    fitWeight: 8,
    conflictPenalty: -22,
    conflicts: [
      'chicken',
  ...
  },
  {
    ...
    conflictMessage:
      'Recipe appears to include animal products, which conflicts with your vegan preference.',
    fitWeight: 10,
    conflictPenalty: -26,
    conflicts: [
      'chicken',
  ...
  },
  {
    id: 'low_carb',
    match: (name) =>
      name.includes('low carb') || name.includes('low-carb') || name.includes('keto'),
    fitWeight: 7,
    conflictPenalty: -12,
    evaluate(recipe, out) {
      if (recipe.carbohydrates != null && recipe.carbohydrates <= 25) {
        ...
        out.reasons.push({
          tag: 'dietary_low_carb',
          message: 'Supports your low-carb preference.',
          weight: 7,
        });
      } else if (recipe.carbohydrates != null && recipe.carbohydrates > 50) {
        out.score -= 12;
        out.warnings.push({
          tag: 'dietary_low_carb_conflict',
          message: `Carbohydrates ${recipe.carbohydrates}g are above a typical low-carb target.`,
          severity: 'info',
        });
      }
    },
  },
  {
    ...
    evaluate(recipe, out) {
      if (recipe.protein != null && recipe.protein >= 20) {
        ...
        out.reasons.push({
          tag: 'dietary_high_protein',
          message: 'Supports your high-protein preference.',
          weight: 8,
        });
      } else if (recipe.protein != null && recipe.protein < 12) {
        out.score -= 8;
        out.warnings.push({
          tag: 'dietary_high_protein_conflict',
          message: `Protein ${recipe.protein}g is lower than a typical high-protein meal target.`,
          severity: 'info',
        });
      }
    },
...
function evaluate(recipe, context = {}) {
  ...
  const dietaryRequirements = Array.isArray(context.dietaryRequirements)
  ...
  if (preferredCuisines.includes(recipe.cuisine_id)) {
    ...
    out.reasons.push({
      tag: 'preferred_cuisine',
      message: 'Matches a cuisine you prefer.',
      weight: PREFERRED_CUISINE_WEIGHT,
    });
  }
  ...
  if (preferredMethods.includes(recipe.cooking_method_id)) {
    ...
    out.reasons.push({
      tag: 'preferred_cooking_method',
      message: 'Uses a cooking method you prefer.',
      weight: PREFERRED_METHOD_WEIGHT,
    });
  }
  ...
  if (preferredRecipes.includes(recipe.id)) {
    ...
    out.reasons.push({
      tag: 'ai_preferred_recipe',
      message: 'AI signal recommends this dish for you.',
      weight: PREFERRED_RECIPE_WEIGHT,
    });
  }
  ...
  if (recipe.dislike === true) {
    ...
    out.warnings.push({
      tag: 'user_disliked',
      message: 'You previously disliked this recipe.',
      severity: 'info',
    });
  } else {
  ...
  if (recentRecipes.includes(recipe.id)) {
    ...
    out.reasons.push({
      tag: 'recent_meal_penalty',
      message: 'Recently served — deprioritised for variety.',
      weight: RECENT_RECIPE_PENALTY,
    });
  }
  ...
  if (goals.prioritizeProtein && recipe.protein != null && recipe.protein >= 15) {
    ...
    out.reasons.push({
      tag: 'goal_high_protein',
      message: 'Supports your higher-protein goal.',
      weight: GOAL_PROTEIN_WEIGHT,
    });
  }
  ...
  if (goals.prioritizeFiber && recipe.fiber != null && recipe.fiber >= 5) {
    ...
    out.reasons.push({
      tag: 'goal_high_fiber',
      message: 'Supports your higher-fibre goal.',
      weight: GOAL_FIBER_WEIGHT,
    });
  }
  ...
  if (goals.limitSugar && recipe.sugar != null) {
    if (recipe.sugar <= 10) {
      ...
      out.reasons.push({
        tag: 'goal_low_sugar',
        message: 'Fits your low-sugar preference.',
        weight: GOAL_SUGAR_WEIGHT,
      });
    } else {
      out.score -= GOAL_SUGAR_WEIGHT;
      out.warnings.push({
        tag: 'goal_sugar_mismatch',
        message: `Sugar ${recipe.sugar}g is above your low-sugar target.`,
        severity: 'info',
      });
    }
  }
  ...
  if (goals.limitSodium && recipe.sodium != null) {
    if (recipe.sodium <= 400) {
      ...
      out.reasons.push({
        tag: 'goal_low_sodium',
        message: 'Fits your low-sodium preference.',
        weight: GOAL_SODIUM_WEIGHT,
      });
    } else {
      out.score -= GOAL_SODIUM_WEIGHT;
      out.warnings.push({
        tag: 'goal_sodium_mismatch',
        message: `Sodium ${recipe.sodium}mg is above your low-sodium target.`,
        severity: 'info',
      });
    }
  }
  ...
  if (goals.targetCalories != null && recipe.calories != null) {
    ...
    if (delta <= 100) {
      ...
      out.reasons.push({
        tag: 'target_calories_close',
        message: `Close to your calorie target (Δ ${Math.round(delta)} kcal).`,
        weight: TARGET_CALORIES_CLOSE,
      });
    } else if (delta <= 250) {
      out.score += TARGET_CALORIES_NEAR;
      out.matchedSignals.push('target_calories_near');
      out.reasons.push({
        tag: 'target_calories_near',
        message: `Near your calorie target (Δ ${Math.round(delta)} kcal).`,
        weight: TARGET_CALORIES_NEAR,
      });
    }
  }
