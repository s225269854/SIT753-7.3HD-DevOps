...
try {
  ...
  if (foodMatch) {
    ...
    // Update the mapping
    const newCustomFoodDict = customFoodDict.replace(foodRegex, `'${FOOD_NAME}': '${CLASS_NAME}'`);
  ...
  if (FOOD_NAME === 'sushi') {
    ...
    const filenameHandlingRegex =
      /# Special handling for sushi\s+if "sushi" in file_name\.lower\(\):[^}]+return "([^"]+)"/s;
    ...
    const originalFilenameHandlingRegex =
      /# Special handling for sushi\s+if "sushi" in original_filename\.lower\(\):[^}]+return "([^"]+)"/s;
    const originalFilenameMatch = content.match(originalFilenameHandlingRegex);

    if (originalFilenameMatch) {
      console.log(
        `Found special handling for sushi in original_filename detection: '${originalFilenameMatch[1]}'`
      );
      console.log(`Updating to '${CLASS_NAME}'...`);
    ...
    const textureHandlingRegex =
      /# Add white\+complex texture classification \(possibly sushi\)[^}]+prediction = '([^']+)'  # Best substitute for sushi/s;
