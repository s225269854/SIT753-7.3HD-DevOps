  ...
  try {
    ...
    if (error) {
      ...
      if (error.message && error.message.includes('does not exist')) {
        ...
        console.error(
          'Please run the SQL script in setup/create_feedback_table.sql in your Supabase SQL Editor.'
        );
      }
    ...
    if (!feedbackData || feedbackData.length === 0) {
      console.log(
        'No feedback data found. Please collect feedback using tools/feedback/collect_feedback.js first.'
      );
      process.exit(0);
    }
    ...
    console.log(
      '   - Use tools/image_classification/add_keywords.js to add more keywords for specific classes'
    );
    console.log('   - Modify color and texture analysis rules in recipeImageClassification.py');
    console.log(
      '   - Consider collecting more samples, especially for classes with high error rates'
    );
