...
const jsonError = (res, status, error, code) => res.status(status).json({ error, code });
...
const verifyCurrentPassword = async (req, res) => {
  try {
    ...
    if (!req.body.password) {
      return jsonError(res, 400, 'Current password is required', 'CURRENT_PASSWORD_REQUIRED');
    }
    ...
    if (!isPasswordValid) {
      return jsonError(res, 401, 'Current password is incorrect', 'CURRENT_PASSWORD_INVALID');
    }
...
const updateUserPassword = async (req, res) => {
  try {
    ...
    if (!req.body.new_password) {
      return jsonError(res, 400, 'New password is required', 'NEW_PASSWORD_REQUIRED');
    }

    const confirmPassword = req.body.confirm_password ?? req.body.new_password;

    if (!confirmPassword) {
      return jsonError(res, 400, 'Confirm password is required', 'CONFIRM_PASSWORD_REQUIRED');
    }
    ...
    if (passwordStrengthError) {
      return jsonError(res, 400, passwordStrengthError.error, passwordStrengthError.code);
    }
