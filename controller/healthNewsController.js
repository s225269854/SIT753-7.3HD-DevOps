...
exports.filterNews = async (req, res) => {
  try {
    ...
    // Build the query
    let query = supabase.from('health_news');
...
exports.updateNews = async (req, res) => {
  ...
  try {
    ...
    if (tags) {
      ...
      const { error: deleteError } = await supabase.from('news_tags').delete().eq('news_id', id);
...
exports.createAuthor = async (req, res) => {
  ...
  try {
    const { data, error } = await supabase.from('authors').insert({ name, bio }).select().single();
