...
const createAndSaveRecipe = async (req, res) => {
  ...
  try {
    ...
    const allergies = recipeIngredients.filter((r) => r.allergy).map((r) => r.recipe_id);

    if (allergies.length > 0) {
      await createRecipe.updateRecipeAllergy(allergies);
    }

    const dislikes = recipeIngredients.filter((r) => r.dislike).map((r) => r.recipe_id);
  ...
  try {
    ...
    await Promise.all(
      recipes.map(async (recipe) => {
        ...
        // Get image URL
        recipe.image_url = await getUserRecipes.getImageUrl(recipe.image_id);
      })
    );
