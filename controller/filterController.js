...
const filterRecipes = async (req, res) => {
  ...
  try {
    ...
    if (
      dietary &&
      !dietaryMapping.some((d) => d.name.toLowerCase().includes(dietary.toLowerCase()))
    ) {
    ...
    const filteredRecipes = recipes.filter((recipe) => {
      ...
      const hasAllergy = recipe.ingredients.some((ingredient) => {
        return (
          ...
          allergyList.some(
            (allergy) => ingredient.allergies_type.name.toLowerCase().includes(allergy) // Check for partial match
          )
        );
      });
