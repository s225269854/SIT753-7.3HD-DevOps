...
const createShoppingListValidation = [
  ...
  body('items.*.ingredient_name').notEmpty().withMessage('Ingredient name cannot be empty'),
  body('items.*.quantity')
    .notEmpty()
    .withMessage('Quantity cannot be empty')
    .isFloat({ min: 0.01 })
    .withMessage('Quantity must be a number greater than 0'),
  body('items.*.unit').notEmpty().withMessage('Unit cannot be empty'),
  body('items.*.measurement').notEmpty().withMessage('Measurement cannot be empty'),
];
...
const addShoppingListItemValidation = [
  ...
  body('meal_tags').optional().isArray().withMessage('Meal tags must be an array'),
  body('estimated_cost')
    .optional()
    .isFloat({ min: 0 })
...
const updateShoppingListItemValidation = [
  ...
  body('purchased').optional().isBoolean().withMessage('Purchased field must be a boolean value'),
  body('quantity')
    .optional()
    .isFloat({ min: 0.01 })
    .withMessage('Quantity must be a number greater than 0'),
