...
const isArrayOfIntegers = (value) => Array.isArray(value) && value.every(Number.isInteger);
...
const healthContextRules = [
  body('health_context').optional().isObject().withMessage('health_context must be an object'),

  // allergies[]
  body('health_context.allergies')
    .optional()
    .isArray()
    .withMessage('health_context.allergies must be an array'),
  body('health_context.allergies.*.referenceId')
    .optional({ nullable: true })
    .isInt({ min: 1 })
    .withMessage('allergy referenceId must be a positive integer'),
  body('health_context.allergies.*.name')
    .optional({ nullable: true })
    .isString()
    .withMessage('allergy name must be a string')
    .isLength({ max: 200 })
    .withMessage('allergy name must be 200 characters or fewer'),
  ...
  body('health_context.allergies.*.notes')
    .optional({ nullable: true })
    .isString()
    .withMessage('allergy notes must be a string')
    .isLength({ max: 1000 })
    .withMessage('allergy notes must be 1000 characters or fewer'),

  // chronic_conditions[]
  body('health_context.chronic_conditions')
    .optional()
    .isArray()
    .withMessage('health_context.chronic_conditions must be an array'),
  body('health_context.chronic_conditions.*.referenceId')
    .optional({ nullable: true })
    .isInt({ min: 1 })
    .withMessage('condition referenceId must be a positive integer'),
  body('health_context.chronic_conditions.*.name')
    .optional({ nullable: true })
    .isString()
    .withMessage('condition name must be a string')
    .isLength({ max: 200 })
    .withMessage('condition name must be 200 characters or fewer'),
  ...
  body('health_context.chronic_conditions.*.notes')
    .optional({ nullable: true })
    .isString()
    .withMessage('condition notes must be a string')
    .isLength({ max: 1000 })
    .withMessage('condition notes must be 1000 characters or fewer'),

  // medications[]
  body('health_context.medications')
    .optional()
    .isArray()
    .withMessage('health_context.medications must be an array'),
  body('health_context.medications.*.name')
    ...
    .isLength({ max: 200 })
    .withMessage('medication name must be 200 characters or fewer'),
  body('health_context.medications.*.dosage')
    .optional()
    .isObject()
    .withMessage('medication dosage must be an object'),
  body('health_context.medications.*.dosage.amount')
    .optional({ nullable: true })
    .isString()
    .withMessage('dosage amount must be a string')
    .isLength({ max: 50 })
    .withMessage('dosage amount must be 50 characters or fewer'),
  body('health_context.medications.*.dosage.unit')
    .optional({ nullable: true })
    .isString()
    .withMessage('dosage unit must be a string')
    .isLength({ max: 50 })
    .withMessage('dosage unit must be 50 characters or fewer'),
  body('health_context.medications.*.frequency')
    .optional()
    .isObject()
    .withMessage('medication frequency must be an object'),
  ...
  body('health_context.medications.*.frequency.interval')
    .optional({ nullable: true })
    .isString()
    .withMessage('frequency interval must be a string')
    .isLength({ max: 100 })
    .withMessage('frequency interval must be 100 characters or fewer'),
  body('health_context.medications.*.frequency.schedule')
    .optional()
    .isArray()
    .withMessage('frequency schedule must be an array'),
  body('health_context.medications.*.frequency.schedule.*')
    .optional()
    .isString()
    .withMessage('each schedule entry must be a string')
    .isLength({ max: 50 })
    .withMessage('each schedule entry must be 50 characters or fewer'),
  body('health_context.medications.*.frequency.asNeeded')
    .optional()
    .isBoolean()
    .withMessage('frequency asNeeded must be a boolean'),
  body('health_context.medications.*.purpose')
    .optional({ nullable: true })
    .isString()
    .withMessage('medication purpose must be a string')
    .isLength({ max: 500 })
    .withMessage('medication purpose must be 500 characters or fewer'),
  body('health_context.medications.*.notes')
    .optional({ nullable: true })
    .isString()
    .withMessage('medication notes must be a string')
    .isLength({ max: 1000 })
    .withMessage('medication notes must be 1000 characters or fewer'),
  body('health_context.medications.*.active')
    .optional()
    .isBoolean()
    .withMessage('medication active must be a boolean'),
];
...
const uiSettingsRules = [
  body('ui_settings').optional().isObject().withMessage('ui_settings must be an object'),
  ...
  body('ui_settings.font_size')
    .optional()
    .isString()
    .withMessage('ui_settings.font_size must be a string')
    .matches(/^\d+(px|rem|em|%)$/)
...
// ─────────────────────────────────────────────────────────────────────────────
exports.validateUserPreferences = [...requiredFoodPreferenceRules];
...
exports.validateNotificationPreferences = [
  body('notification_preferences')
    .notEmpty()
    .withMessage('notification_preferences object is required')
    .isObject()
    .withMessage('notification_preferences must be an object'),
  body('notification_preferences.mealReminders')
    .optional()
    .isBoolean()
    .withMessage('mealReminders must be a boolean'),
  body('notification_preferences.waterReminders')
    .optional()
    .isBoolean()
    .withMessage('waterReminders must be a boolean'),
  body('notification_preferences.healthTips')
    .optional()
    .isBoolean()
    .withMessage('healthTips must be a boolean'),
  ...
  body('notification_preferences.systemUpdates')
    .optional()
    .isBoolean()
    .withMessage('systemUpdates must be a boolean'),
];
