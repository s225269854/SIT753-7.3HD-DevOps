...
describe('Water Intake API', () => {
  ...
  describe('POST /api/water-intake', () => {
    ...
    it('should return 400 if user_id is missing', async () => {
      const res = await request(BASE_URL).post('/api/water-intake').send({ glasses_consumed: 3 });
    ...
    it('should return 400 if glasses_consumed is missing', async () => {
      const res = await request(BASE_URL).post('/api/water-intake').send({ user_id: testUserId });
