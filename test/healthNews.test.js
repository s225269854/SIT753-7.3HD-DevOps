...
describe('Health News API', () => {
  ...
  describe('POST /api/health-news', () => {
    it('should create a news article', async () => {
      const res = await request(BASE_URL).post('/api/health-news').send({
        title: 'New Jest News',
  ...
  describe('PUT /api/health-news', () => {
    ...
    it('should return 400 if id missing', async () => {
      const res = await request(BASE_URL).put('/api/health-news').send({ title: 'Updated Title' });
      expect(res.statusCode).toBe(400);
      expect(res.body.success).toBe(false);
    });
  });
  ...
  describe('DELETE /api/health-news', () => {
    it('should delete a news article', async () => {
      const res = await request(BASE_URL).delete('/api/health-news').query({ id: createdNewsId });
      expect(res.statusCode).toBe(200);
      expect(res.body.success).toBe(true);
