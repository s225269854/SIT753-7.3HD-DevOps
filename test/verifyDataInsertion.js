...
async function verifyDataInsertion() {
  ...
  try {
    ...
    if (usersError) {
      ...
      users.forEach((user) => {
        console.log(
          `   - ID: ${user.user_id}, Name: ${user.name}, Email: ${user.email}, Registered: ${user.registration_date}`
        );
      });
    } else {
    ...
    if (shoppingError) {
      ...
      shoppingLists.forEach((list) => {
        console.log(
          `   - ID: ${list.id}, User ID: ${list.user_id}, Name: ${list.name}, Cost: $${list.estimated_total_cost}, Created: ${list.created_at}`
        );
      });
    } else {
    ...
    const { data: items, error: itemsError } = await supabase
      ...
      .select(
        'id, shopping_list_id, ingredient_name, category, quantity, unit, measurement, estimated_cost, created_at'
      )
      .order('created_at', { ascending: false })
      .limit(10);
    ...
    if (itemsError) {
      ...
      items.forEach((item) => {
        console.log(
          `   - ID: ${item.id}, List ID: ${item.shopping_list_id}, ${item.ingredient_name} (${item.category}), Qty: ${item.quantity}${item.measurement}, Cost: $${item.estimated_cost}`
        );
      });
    } else {
    ...
    if (shoppingLists && shoppingLists.length > 0 && items && items.length > 0) {
      ...
      if (orphanedItems.length > 0) {
        console.log(
          '⚠️ Found orphaned items (shopping_list_id not in shopping_lists):',
          orphanedItems
        );
      } else {
      ...
      if (orphanedLists.length > 0) {
        console.log(
          '⚠️ Found orphaned shopping lists (user_id not in users):',
          orphanedLists.map((l) => l.id)
        );
      } else {
    ...
    if (shoppingLists && shoppingLists.length > 0) {
      const totalCost = shoppingLists.reduce(
        (sum, list) => sum + (list.estimated_total_cost || 0),
        0
      );
    ...
    const testLists = shoppingLists
      ? shoppingLists.filter((list) => list.name.includes('Test'))
      : [];
