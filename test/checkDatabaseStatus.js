...
async function checkDatabaseStatus() {
  ...
  try {
    ...
    try {
      ...
      if (shoppingError) {
        console.log(
          '⚠️ shopping_lists table query failed (table might not exist):',
    ...
    if (specificError) {
      if (specificError.code === 'PGRST116') {
        console.log(
          '❌ User with user_id=1 does not exist - this explains the foreign key constraint violation!'
        );
    ...
    console.log(
      '3. Check your database schema to ensure foreign key constraints are properly set up'
    );
