...
async function fetchIngredientSubstitutions(ingredientId, options = {}) {
  ...
  if (options.allergies !== undefined) {
    if (!Array.isArray(options.allergies)) {
      ...
      if (typeof options.allergies === 'string') {
        try {
          options.allergies = options.allergies
            .split(',')
  ...
  if (options.dietaryRequirements !== undefined) {
    if (!Array.isArray(options.dietaryRequirements)) {
      ...
      if (typeof options.dietaryRequirements === 'string') {
        try {
          options.dietaryRequirements = options.dietaryRequirements
            .split(',')
            .map((id) => parseInt(id.trim()))
            .filter((id) => !isNaN(id));
          console.log(
            `Parsed dietary requirements from string: ${JSON.stringify(options.dietaryRequirements)}`
          );
        } catch (parseError) {
      ...
      options.dietaryRequirements = options.dietaryRequirements
        ...
        .filter((id) => !isNaN(id));
      console.log(
        `Validated dietary requirements array: ${JSON.stringify(options.dietaryRequirements)}`
      );
    }
  }
  ...
  if (options.healthConditions !== undefined) {
    if (!Array.isArray(options.healthConditions)) {
      ...
      if (typeof options.healthConditions === 'string') {
        try {
          options.healthConditions = options.healthConditions
            .split(',')
            .map((id) => parseInt(id.trim()))
            .filter((id) => !isNaN(id));
          console.log(
            `Parsed health conditions from string: ${JSON.stringify(options.healthConditions)}`
          );
        } catch (parseError) {
  ...
  try {
    ...
    console.log(
      `Found original ingredient: ${originalIngredient.name} (Category: ${originalIngredient.category})`
    );
    ...
    let query = supabase
      .from('ingredients_new')
      .select(
        'ingredient_id, name, category, calories, fat, carbohydrates, protein, fiber, sodium, sugar'
      )
    ...
    if (options.allergies && Array.isArray(options.allergies) && options.allergies.length > 0) {
      try {
        ...
        if (Array.isArray(options.allergies)) {
          ...
          console.error(
            'Allergies is not an array, this should not happen as controller should convert it'
          );
        ...
        if (validAllergyIds.length > 0) {
          ...
          if (allergenInfo && allergenInfo.length > 0) {
            ...
            const { data: ingredientsWithAllergens, error: ingredientAllergenError } =
              await supabase
            ...
            if (ingredientsWithAllergens && ingredientsWithAllergens.length > 0) {
              ...
              console.log(
                `Found ${ingredientsToExclude.length} ingredients to exclude due to allergens`
              );
            }
    ...
    if (
      options.dietaryRequirements &&
      Array.isArray(options.dietaryRequirements) &&
      options.dietaryRequirements.length > 0
    ) {
      try {
        console.log(
          `Processing dietary requirements filter with ${options.dietaryRequirements.length} items`
        );
        ...
        if (Array.isArray(options.dietaryRequirements)) {
          ...
          console.error(
            'DietaryRequirements is not an array, this should not happen as controller should convert it'
          );
          // Fallback handling just in case
          validDietaryIds = [];
        }
        ...
        if (validDietaryIds.length > 0) {
          ...
          if (dietaryRequirementInfo && dietaryRequirementInfo.length > 0) {
            ...
            if (dietaryIngredientsError) {
              console.error(
                'Error fetching dietary requirement ingredients mapping:',
                dietaryIngredientsError
              );
            ...
            if (dietaryIngredients && dietaryIngredients.length > 0) {
              console.log(
                `Found ${dietaryIngredients.length} dietary requirement-ingredient mappings`
              );
              ...
              validDietaryIds.forEach((id) => {
                console.log(
                  `Dietary requirement ${id}: ${includeIngredients[id].length} include ingredients, ${avoidIngredients[id].length} avoid ingredients`
                );
              });
              ...
              if (allAvoidIngredients.length > 0) {
                console.log(
                  `Excluding ${allAvoidIngredients.length} ingredients to avoid based on dietary requirements`
                );
              ...
              if (hasIncludeRecommendations) {
                ...
                if (includeForAllRequirements && includeForAllRequirements.length > 0) {
                  console.log(
                    `Prioritizing ${includeForAllRequirements.length} ingredients recommended for all dietary requirements`
                  );
              ...
              console.log(
                'No dietary requirement-ingredient mappings found, falling back to default filtering'
              );
                ...
                switch (dietaryReq.requirement_name.toLowerCase()) {
                  case 'vegetarian':
                    ...
                    if (!vegError && vegetarianIngredients) {
                      dietaryIngredientMapping[dietaryReq.dietary_requirement_id] =
                        vegetarianIngredients.map((ing) => ing.ingredient_id);
                    }
                    break;
                  ...
                  case 'vegan':
                    ...
                    if (!veganError && veganIngredients) {
                      dietaryIngredientMapping[dietaryReq.dietary_requirement_id] =
                        veganIngredients.map((ing) => ing.ingredient_id);
                    }
                    break;
                  ...
                  case 'gluten-free':
                    ...
                    if (!gfError && glutenFreeIngredients) {
                      dietaryIngredientMapping[dietaryReq.dietary_requirement_id] =
                        glutenFreeIngredients.map((ing) => ing.ingredient_id);
                    }
                    break;
                  ...
                  default:
                    ...
                    if (!matchError && matchingIngredients) {
                      dietaryIngredientMapping[dietaryReq.dietary_requirement_id] =
                        matchingIngredients.map((ing) => ing.ingredient_id);
                    }
                    break;
                }

                console.log(
                  `Mapped dietary requirement ${dietaryReq.requirement_name} to ${dietaryIngredientMapping[dietaryReq.dietary_requirement_id]?.length || 0} ingredients`
                );
              }
              ...
              if (validIngredientIds.length > 0) {
                console.log(
                  `Including ${validIngredientIds.length} ingredients that match all dietary requirements`
                );
    ...
    if (
      options.healthConditions &&
      Array.isArray(options.healthConditions) &&
      options.healthConditions.length > 0
    ) {
      try {
        console.log(
          `Processing health conditions filter with ${options.healthConditions.length} items`
        );
        ...
        if (Array.isArray(options.healthConditions)) {
          ...
          console.error(
            'HealthConditions is not an array, this should not happen as controller should convert it'
          );
          // Fallback handling just in case
          validHealthIds = [];
        }
        ...
        if (validHealthIds.length > 0) {
          ...
          const { data: healthConditionInfo, error: healthError } = await supabase
            ...
            .select(
              'condition_id, name, description, recommended_foods, restricted_foods, severity_level'
            )
          ...
          if (healthConditionInfo && healthConditionInfo.length > 0) {
            ...
            if (conditionIngredientsError) {
              console.error(
                'Error fetching condition ingredients mapping:',
                conditionIngredientsError
              );
            ...
            if (conditionIngredients && conditionIngredients.length > 0) {
              ...
              validHealthIds.forEach((id) => {
                console.log(
                  `Condition ${id}: ${includeIngredients[id].length} include ingredients, ${avoidIngredients[id].length} avoid ingredients`
                );
              });
              ...
              if (allAvoidIngredients.length > 0) {
                console.log(
                  `Excluding ${allAvoidIngredients.length} ingredients to avoid based on health conditions`
                );
                query = query.not('ingredient_id', 'in', `(${allAvoidIngredients.join(',')})`);
              }
              ...
              if (hasIncludeRecommendations) {
                ...
                if (includeForAllConditions && includeForAllConditions.length > 0) {
                  console.log(
                    `Prioritizing ${includeForAllConditions.length} ingredients recommended for all health conditions`
                  );
              ...
              console.log(
                'No condition-ingredient mappings found, using health condition metadata'
              );
              ...
              if (allRestrictedFoods.length > 0) {
                console.log(
                  `Using ${allRestrictedFoods.length} restricted foods from health conditions metadata`
                );
              ...
              if (allRecommendedFoods.length > 0) {
                console.log(
                  `Using ${allRecommendedFoods.length} recommended foods from health conditions metadata`
                );
                ...
                const { data: recommendedIngredients, error: recommendedError } =
                ...
                if (
                  ...
                  console.log(
                    `Found ${recommendedIds.length} ingredients matching recommended foods`
                  );
