...
function convertUnits(value, fromMeasurement, toMeasurement) {
  ...
  if (toMeasurement === 'N/A') {
    ...
    if (conversions.weight[fromMeasurement] && conversions.weight[toMeasurement]) {
      result.unit =
        value * (conversions.weight[toMeasurement] / conversions.weight[fromMeasurement]);
      return result;
    } else if (conversions.liquid[fromMeasurement] && conversions.liquid[toMeasurement]) {
      result.unit =
        value * (conversions.liquid[toMeasurement] / conversions.liquid[fromMeasurement]);
      return result;
    } else {
      throw new Error('Invalid unit conversion');
    }
...
function estimateIngredientsCost(ingredients, ingredients_price) {
  ...
  if (
    ingredients.id.length === ingredients.quantity.length &&
    ingredients.id.length === ingredients.measurement.length
  ) {
