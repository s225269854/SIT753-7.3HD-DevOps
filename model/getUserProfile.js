...
async function decryptSensitiveFields(profile) {
  ...
  const decryptedContact = profile.contact_number
    ? await (async () => {
  ...
  const decryptedAddress = profile.address
    ? await (async () => {
...
async function getImageUrl(image_id) {
  try {
    ...
    let { data } = await supabase.from('images').select('*').eq('id', image_id);
    if (data[0] != null) {
...
async function resolveImageUrl(file_name) {
  ...
  const { data: publicData } = supabase.storage.from('images').getPublicUrl(file_name);
