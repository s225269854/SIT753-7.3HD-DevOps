...
async function getUserAllergen(user_id, isFromRecipe = false) {
  if (isFromRecipe) {
    ...
    const user_allergen_ingredient_ids = [
      ...new Set(user_allergen_result.map((item) => item.ingredient_id)),
    ];
    const user_allergen_ingredients = await getIngredients(user_allergen_ingredient_ids);
    const user_allergen_ingredient_names = user_allergen_ingredients.map((item) =>
  ...
  const user_allergen_ingredient_names = user_allergen_result.map((item) =>
    item.ingredients.name.toLowerCase()
  );
  return user_allergen_ingredient_names;
}
