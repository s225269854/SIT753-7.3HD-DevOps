...
async function estimateCost(recipe_id, desired_servings, exclude_ids) {
  ...
  if (desired_servings > 0) {
    ingredients_result = await getRecipeIngredients.getScaledIngredientsByServing(
      recipe_id,
      desired_servings
    );
  } else {
  ...
  if (!isFull) {
    ...
    ingredients.measurement = ingredients.measurement.filter(
      (id, i) => !exclude_indices.includes(i)
    );
  }
  ...
  const { lowPriceRequiredIngredients, highPriceRequiredIngredients } =
  ...
  const { estimatedCost, lowPriceID, highPriceID } = getEstimatedCost.prepareResponseData(
